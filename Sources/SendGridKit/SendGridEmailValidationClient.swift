...
/// A client for validating emails using the SendGrid API.
public struct SendGridEmailValidationClient: Sendable {
    ...
    /// Initialize a new `SendGridClientEmailValidationClient`
    ///
    /// - Parameters:
    ...
    func getBulkValidationUploadURL(fileType: BulkEmailValidationUploadURLRequest.FileType) async throws
        -> BulkEmailValidationUploadURLResponse
    {
        var headers = HTTPHeaders()
        headers.add(name: "Authorization", value: "Bearer \(apiKey)")
        headers.add(name: "Content-Type", value: "application/json")
    ...
    public func uploadBulkValidationFile(
        fileData: Data,
        fileType: BulkEmailValidationUploadURLRequest.FileType
    ) async throws -> UploadBulkValidationFileResult {
